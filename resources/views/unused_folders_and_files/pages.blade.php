      ...
      <div class="page-content">
        <div>
          <ol class="breadcrumb h6">
            ...
            <li><a href="{{URL(App::getLocale().'/page/'.$page->page_category_group_id )}}"
                title="{{ $page->category_name }}">{{ $page->category_name }}</a></li>
    ...
    <div class="col-md-3" style="padding-top: 50px;">
      <div class="col menu-item-structure">
        ...
        <div class="list-group">
          ...
          @if ($row->page_group_id == $page->page_group_id)
          <a href="{{url(App::getLocale().'/page/'.$row->page_category_group_id.'/'.$row->page_group_id)}}"
            class="list-group-item active">
            {{ $row->title }}</a>

          @else
          <a href="{{url(App::getLocale().'/page/'.$row->page_category_group_id.'/'.$row->page_group_id)}}"
            class="list-group-item">{{ $row->title }}</a>
          @endif
