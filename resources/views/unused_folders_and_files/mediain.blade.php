                    ...
                    <div class="page-content">
                        <div>
                            <ol class="breadcrumb h6">
                                ...
                                <li><a href="{{ URL(App::getLocale().'/video/'.$curcat->group) }}">@lang('blog.video')</a></li>
                            </ol>
                        </div>
                    </div>
                        ...
                        <div class="list-group">
                            @foreach($newscat as $value)
                                @if($curcat->group == $value->group)
                                    <a href="{{ URL(App::getLocale().'/video/'.$value->group) }}" class="list-group-item active">{{$value->title}}</a>
                                @else
                                    <a href="{{ URL(App::getLocale().'/video/'.$value->group) }}" class="list-group-item">{{$value->title}}</a>
                                @endif
                            @endforeach
